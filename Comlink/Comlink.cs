...
namespace Comlink
{
    ...
    {
        public static void Expose<T>(this WebView2 window, T target) => Expose(target, WindowEndpoint(window));
        ...
        public static Endpoint WindowEndpoint(WebView2 window)
        {
        ...
        public static void Expose<T>(T target, Endpoint endpoint)
        {
            endpoint.Message += (s, a) =>
            {
                try
                {
                    ...
                    Message message = JsonSerializer.Deserialize<Message>(a.WebMessageAsJson, options) ??
                                      throw new Exception(
                                          "Invalid message received, could not deserialize json to instance of 'Message'");

                    (String id, MessageType type, String[] path) = message;
                    Object?[] args = message.ArgumentList?.Select(FromWireValue).ToArray() ??
                                     (message.Value != null ? new[] {FromWireValue(message.Value)} : new Object?[0]);

                    (Object? owner, Object? value, MemberInfo? member) =
                        path.Aggregate<String, (Object? owner, Object? value, MemberInfo?)?>(
                            (null, target, null),
                            (obj, prop) =>
                            {
                                MemberInfo? member = obj?.value?.GetType().GetMember(prop,
                            ...
                            }) ?? throw new Exception($"Unable to access the path '{path}'");
                    ...
                    Object? returnValue = type switch
                    {
